                    ...
                    <div class="bg-white shadow-md rounded my-6">
                        <table class="text-left w-full border-collapse">
                            ...
                            <tbody> @foreach ($movies as $movie) <tr class="hover:bg-grey-lighter">
                                    ...
                                    <td class="py-4 px-6 border-b border-grey-light">{{ $movie['format'] }}</td>
                                    <td class="py-4 px-6 border-b border-grey-light flex items-center justify-end space-x-2">
                                        <button onclick="showMovieInfo({{ $movie['id'] }})" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Details</button>
                                        <form action="/delete-movie/{{ $movie['id'] }}" method="POST" onsubmit="return confirm('Are you sure?');" class="inline">
